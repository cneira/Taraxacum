...
enum class HTTP_METHOD : std::int8_t {
...
public:
    ...
    virtual void Measure(const std::string data) = 0;
...
public:
    ...
    void Measure(const std::string data) { std::cout << " Uservice Measure" << std::endl; }
...
public:
    ...
    void Measure(const std::string data) {
        std::cout << "I'm in Provider" << std::endl;
        usvc->Measure(data);
    }
